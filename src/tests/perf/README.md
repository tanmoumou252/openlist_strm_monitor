...
以下数字仅来自独立模拟 benchmark，不代表生产 reconciliation 的验证结果；实际结果受机器、磁盘、Python、SQLite 和数据分布影响，不承诺固定倍数或固定秒数。

- 8507条 / 59.5秒 = 6.9ms/条 (历史环境日志)
...
|------|----------|-----------|------|
| 8507条耗时 | 11.4秒 | **0.71秒** | 历史环境观测值 |
| 验证记录数 | 8507 (100%) | **42 (0.5%)** | -99.5% |
| 跳过记录数 | 0 | **8465** | — |
| 正确性 (digest) | — | ✅ 一致 | 无回归 |

历史 benchmark 曾在该环境观测到 0.71 秒；这不是生产验证，也不应作为固定性能承诺。
