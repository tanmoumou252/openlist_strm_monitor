...
_src_dir = Path(__file__).parent.resolve().parent
if str(_src_dir) not in sys.path:
    sys.path.insert(0, str(_src_dir))
_current_dir = _src_dir
...
def main():
    ...
    tmdb_watchlist_db_path = project_root / "tmdb_watchlist.db"
    if tmdb_watchlist_db_path.exists():
        try:
            from tmdb_watchlist_db import TmdbWatchlistDb
            migrate_wdb = TmdbWatchlistDb(str(tmdb_watchlist_db_path))
            config.update_from_db(migrate_wdb)
            log.info(f"已从 DB 加载配置覆盖，Host: {config.webdav.host}")
        except Exception as exc:
            log.warning(f"从 DB 加载配置失败: {exc}")
    ...
    if not client.login(force=True):
        error_msg = client.last_error_message or "未知错误"
        error_type = client.last_error_type or "unknown"
        log.error("登录失败: %s (类型: %s)，跳过后续测试", error_msg, error_type)
        return
