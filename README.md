...
2.  **事件驱动**：
    ...
    *   **二次删除**: 经过webdav 对strm驱动目录执行的一切操作都可以触发openlist的更新钩子,所以删除strm对应的源文件这个操作也会触发钩子导致刚刚删除的strm又被生成了,所以设计了二次删除strm
3.  **自愈体检**：
...
*   **WebDAV 兼容性：** 针对 Alist 进行了深度优化，但也兼容其他标准 WebDAV 服务。若使用 `MOVE` 模式，请确保 WebDAV 服务端支持 `MOVE` 指令。
