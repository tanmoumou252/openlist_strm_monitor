...
`openlist_strm_bridge` 是一个专为 **OpenList STRM 引擎同步模式** 量身打造的**智能防灾同步中间件**。

它的核心职责是：作为 OpenList 与媒体库（Emby / Jellyfin / Plex）之间的协调中枢，打通 STRM 的“**生成 -> 刮削消费 -> 重命名整理 -> 删除 -> 云端联动 -> 冗余回收**”整条闭环链路。在此过程中，提供极强的自我保护能力，防止手误或网络异常导致的数据灾难。
...
3. **网盘掉线断路器（Fail-Safe 防误删）**
   普通的同步软件在网盘掉线时，会以为云端文件被删从而清空本地媒体库。本程序在执行“失效记录清理”前，必须强制通过 API 进行**网络探活**。一旦发现挂载点掉线或网络异常，立刻锁死清理操作，最大程度保护媒体库资产。
4. **优胜劣汰的单实例去重（防重复刮削）**
...
- **A 区 (生肉区)**：OpenList 引擎同步模式的输出目录。程序在此区**只读不写**，仅用作提取 WebDAV 映射和建立身份指纹。
- **B 区 (熟肉区)**：Emby / Jellyfin / Plex 真正扫描的目录。用户在此区自由改名、整理、删除。程序将用户的操作翻译为云端 API 指令。
...
| `DEBUG` | 用于排查指纹计算、血统拦截细节、重命名追踪溯源等。 |
| `WARNING` | 可恢复的异常，如单兵脱群观察期、劣质文件隔离等。 |
...
2. **测试验证**：正式接入庞大媒体库前，建议先用测试目录验证：`A -> B 优选同步`、`B 跨级移动血统拦截`、`B 删除联动云端回收站`。
3. **数据库重建**：如果大规模修改了 OpenList 的存储结构，建议清空 `bridge.db` 让程序重新逆向建库。
