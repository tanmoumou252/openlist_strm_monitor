import{F as e,o as t,p as n,s as r}from"./core-353FXnzu.js";var i=`tmdb`,a={sync:`同步`,sync_start:`同步启动`,sync_done:`同步完成`,sync_error:`同步失败`,match_refresh_start:`收录刷新启动`,match_refresh:`收录刷新`,match_refresh_done:`收录刷新完成`,match_refresh_error:`收录刷新失败`,match_override:`收录覆盖`,match:`收录匹配`,match_done:`收录匹配完成`,match_error:`收录匹配失败`,configure:`配置保存`,config_save:`配置保存`,config_update:`配置更新`,restart:`重启`,webui_restart:`WebUI 重启`,login:`登录`,logout:`登出`,add:`新增`,update:`更新`,delete:`删除`,fetch:`拉取`,search:`搜索`,cache_clear:`清理缓存`,cache_hit:`缓存命中`,cache_miss:`缓存未命中`,api_call:`API 调用`,api_error:`API 错误`,rate_limit:`速率限制`,auth:`认证`,token_refresh:`令牌刷新`,watchlist_sync:`待看列表同步`,watchlist_refresh:`待看列表刷新`,info:`信息`,warn:`警告`,error:`错误`,success:`成功`};async function o(e){await s(e)}async function s(o){let l=await n(i===`tmdb`?`/api/tmdb/logs`:`/api/logs`),u,d;i===`tmdb`?(u=l.logs||[],d=l.count||u.length):(u=(l.lines||[]).map(e=>({msg:e})),d=l.count||u.length);let f={info:`var(--primary)`,success:`#188038`,warn:`#e37400`,error:`#d93025`},p={info:`info`,success:`check`,warn:`warn`,error:`error`},m={info:`信息`,success:`成功`,warn:`警告`,error:`错误`},h=u.map(n=>{if(i===`main`)return`<tr><td>${t(n.msg)}</td></tr>`;{let i=n.level||`info`,o=f[i]||`var(--text-muted)`,s=p[i]||`info`,c=m[i]||i,l=a[n.op]||n.op||`-`;return`<tr>
        <td style="white-space:nowrap">${t(n.ts?r(n.ts):`-`)}</td>
        <td><span style="color:var(--primary)">${t(l)}</span></td>
        <td><span style="color:${o};display:inline-flex;align-items:center;gap:4px">${e(s)} ${t(c)}</span></td>
        <td>${t(n.msg)}</td>
...
<div class="log-type-toggle" id="log-type-toggle">
  <button data-log-type="tmdb" class="${_}">${t(y)}</button>
  <button data-log-type="main" class="${g}">${t(b)}</button>
</div>
<div class="toolbar">
  <button class="toolbar-btn" id="logs-refresh">${e(`refresh`)} 刷新当前日志</button>
  <button class="toolbar-btn secondary" id="logs-download">${e(`download`)} ${t(v)}</button>
