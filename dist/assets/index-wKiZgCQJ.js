const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./dashboard-D2LcApJ9.js","./core-DtJL5LfY.js"])))=>i.map(i=>d[i]);
import{A as e,C as t,I as n,L as r,M as i,N as a,P as o,R as s,S as c,d as l,f as u,g as d,h as f,i as p,k as m,l as h,m as g,n as _,o as v,p as y,r as b,z as x}from"./core-DtJL5LfY.js";(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var S={webdav_host:`OpenList 的 WebDAV 服务地址。
格式：http://IP:端口/dav
...
留空使用默认值。
修改并保存后，重启 WebUI / 主程序即按此路径与上面设置的级别写日志。`,refresh_full_audit_interval_days:`每隔多少天执行一次 A→B 全量审计。
设为 0 可关闭周期审计。
保存后即时生效。`,refresh_log_level:`刷新日志级别控制：DEBUG / INFO / WARNING。
保存后即时生效。`};function C(e,t=!1){let r=S[e.replace(/-/g,`_`)];if(!r)return``;let i=v(r);return`<span class="ol-help-icon${t?` tooltip-below`:``}" data-tooltip="${i}" aria-label="帮助">${n(`info`)}</span>`}async function w(e){let t={};try{let e=await g(`/api/webui/config/openlist`);e.success&&e.config&&(t=e.config)}catch{}try{let e=await g(`/api/openlist/strm-engines`);e.success&&(d.availableEngines=e.engines||[])}catch{d.availableEngines=[]}d.strmEngines=[];try{t.strm_engines&&(d.strmEngines=JSON.parse(t.strm_engines))}catch{d.strmEngines=[]}d.strmEngines.length||(d.strmEngines=[{engine:``,monitored_paths:[]}]),d.configured=!!(t.webdav_host&&t.webdav_host.trim()),F();let r=[];try{t.refresh_paths&&(r=JSON.parse(t.refresh_paths))}catch{r=[]}d.refreshPaths=r.slice();let i=[];try{t.a_b_mappings&&(i=JSON.parse(t.a_b_mappings))}catch{i=[]}d.abMappings=i;function a(e,t,n,r,i=`text`,a=!1,o=!1,s=``,c=``){return p(e,t,n,{placeholder:r,type:i,persistLabel:a,readOnly:o,helpIcon:C(c||e.replace(/^ol-/,``)),helperText:s})}function o(e,t,n,r,i=!1,a=``,o=``){let s=a||e.replace(/^ol-/,``),c=n!=null&&String(n).trim()!==``,l=c||i?`floating-label is-shown is-floating${c?` is-filled`:``}`:`floating-label`,u=i?` data-persist-label="1"`:``,d=r.map(e=>{let t=e.value===n?` selected`:``;return`<option value="${v(e.value)}"${t}>${v(e.label)}</option>`}).join(``);return`
      <div class="floating-field" data-field="${e}">
        <div class="field-control">
          <label class="${l}" data-role="label" for="${e}">${v(t)}${C(s)}</label>
          <select id="${e}" class="ol-select"${u}>${d}</select>
        </div>${o?`<div class="field-helper-text">${v(o)}</div>`:``}
      </div>`}function s(e,t,n,r=``,i=``){let a=r||e.replace(/^ol-/,``);return`<div class="toggle-row">
      <span>${v(t)}${C(a)}</span>
    ...
    </div>${i?`<div class="field-helper-text">${v(i)}</div>`:``}`}function c(){let e=`<div class="strm-engine-wrap"><span class="monitored-paths-help">`+C(`monitored_paths`)+`</span>`;e+=`<table class="strm-engine-table"><thead><tr><th style="width:35%">STRM 引擎入口</th><th>监控目录</th><th style="width:60px">操作</th></tr></thead><tbody>`,d.strmEngines.forEach((t,n)=>{e+=l(n,t,d.strmEngines.length)}),e+=`</tbody></table>`;let t=d.configured?``:` disabled`;return e+=`<div class="table-actions">
    ...
    <div class="field-grid">
      ${a(`ol-webdav-host`,`WebDAV 地址`,t.webdav_host||``,`http://127.0.0.1:5244/dav`)}
      ${a(`ol-webdav-user`,`用户名`,t.webdav_user||``,`admin`)}
      ${a(`ol-webdav-password`,`密码`,t.webdav_password||``,`输入密码`,`password`)}
      ${a(`ol-webdav-totp-secret`,`2FA 密钥`,t.webdav_totp_secret||``,`留空则不使用 2FA`,`password`)}
      ${a(`ol-c-root`,`C 区根目录`,t.c_root||e.c_root||``,`请填写 C 区根目录的绝对路径`)}
    </div>
  ...
  </div>`,u+=`<div class="openlist-2col-grid">`;let f=(t.refresh_enabled||`true`).toLowerCase()===`true`,m=t.refresh_interval_minutes||`10`,h=t.refresh_depth||`5`,_=t.refresh_full_audit_interval_days||`7`;u+=`<div class="config-section"><h3>${n(`refresh`)} 刷新配置</h3>
    ${s(`ol-refresh-enabled`,`启用主动刷新`,f)}
    <div class="field-grid">
      ${a(`ol-refresh-interval`,`刷新间隔 (分钟)`,m,`10`,`number`,!1,!1,S.refresh_interval_minutes,`refresh_interval_minutes`)}
      ${a(`ol-refresh-depth`,`刷新深度`,h,`5`,`number`)}
      ${a(`ol-refresh-audit-days`,`全量审计周期 (天，0=关闭)`,_,`7`,`number`,!1,!1,S.refresh_full_audit_interval_days,`refresh_full_audit_interval_days`)}
      ${o(`ol-refresh-log-level`,`刷新日志级别`,t.refresh_log_level||`INFO`,[{value:`DEBUG`,label:`DEBUG — 调试`},{value:`INFO`,label:`INFO — 信息（推荐）`},{value:`WARNING`,label:`WARNING — 警告`}],!1,`refresh_log_level`,S.refresh_log_level)}
    </div>
  ...
  </div>`;let y=(t.behavior_sync_on_startup||`false`).toLowerCase()===`true`,b=t.behavior_action||`MOVE`;u+=`<div class="config-section"><h3>${n(`settings`)} 行为配置</h3>
    <div class="field-grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:6px 12px">
      ${o(`ol-action`,`删除动作`,b,[{value:`MOVE`,label:`MOVE — 移动到回收站（推荐）`},{value:`DELETE`,label:`DELETE — 直接删除（危险）`}],!1,`behavior_action`,S.behavior_action)}
      ${a(`ol-trash-dir`,`回收站目录名`,t.behavior_trash_dir_name||`trash`,`strm_回收站`,`text`,!1,!1,S.behavior_trash_dir_name,`behavior_trash_dir_name`)}
      ${a(`ol-ghost-protect`,`Ghost 保护时间 (秒)`,t.behavior_ghost_protect_seconds||`300`,`300`,`number`,!1,!1,S.behavior_ghost_protect_seconds,`behavior_ghost_protect_seconds`)}
      ${a(`ol-restore-delay`,`A→B 恢复延迟 (秒)`,t.behavior_a_to_b_restore_delay_seconds||`30`,`30`,`number`,!1,!1,S.behavior_a_to_b_restore_delay_seconds,`behavior_a_to_b_restore_delay_seconds`)}
      ${a(`ol-startup-wait`,`启动等待时间 (秒)`,t.behavior_sync_on_startup_wait||`0`,`0`,`number`,!1,!1,S.behavior_sync_on_startup_wait,`behavior_sync_on_startup_wait`)}
    </div>
    ${s(`ol-sync-startup`,`启动时全量同步`,y,`behavior_sync_on_startup`,S.behavior_sync_on_startup)}
  </div>`;let x=t.log_level||`INFO`;u+=`<div class="config-section"><h3>${n(`log`)} 日志配置</h3>
    <div class="field-grid">
      ${o(`ol-log-level`,`日志级别`,x,[{value:`DEBUG`,label:`DEBUG — 调试`},{value:`INFO`,label:`INFO — 信息（推荐）`},{value:`WARNING`,label:`WARNING — 警告`},{value:`ERROR`,label:`ERROR — 错误`}])}
      ${a(`ol-log-max-size`,`日志最大大小 (MB)`,t.log_max_size_mb||`2`,`2`,`number`,!1,!1,S.log_max_size_mb,`log_max_size_mb`)}
      ${a(`ol-log-backup-count`,`历史日志数量`,t.log_backup_count||`5`,`5`,`number`)}
      ${a(`ol-log-path`,`日志保存路径`,t.log_file||e.log_file||``,`strm_bridge.log`,`text`,!1,!1,S.log_file,`log_file`)}
    </div>
  </div>`;let w=d.strmEngines.filter(e=>e.engine),T=[];w.forEach(e=>{let t=d.availableEngines.find(t=>t.mount_path===e.engine);t&&t.local_path&&!T.includes(t.local_path)&&T.push(t.local_path)}),u+=`<div class="config-section"><h3>${n(`folder`)} A↔B 目录映射</h3>
    ...
    <div class="ab-mappings-display" id="ab-mappings-display">
      ${T.length?T.map((e,n)=>{let r=``;if(t.a_b_mappings)try{let n=JSON.parse(t.a_b_mappings).find(t=>t.a_root===e);n&&(r=n.b_root||``)}catch{}return`
        <div class="ab-mapping-row">
          <span class="a-folder-chip">${v(e)}</span>
