const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./dashboard-Vi-mQwLK.js","./core--V5ingeY.js"])))=>i.map(i=>d[i]);
import{A as e,C as t,I as n,L as r,M as i,N as a,P as o,R as s,S as c,d as l,f as u,g as d,h as f,i as p,k as m,l as h,m as g,n as _,o as v,p as y,r as b,z as x}from"./core--V5ingeY.js";(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var S={webdav_host:`OpenList 的 WebDAV 服务地址。
格式：http://IP:端口/dav
...
修改并保存后，重启 WebUI / 主程序即按此路径与上面设置的级别写日志。`};function C(e,t=!1){let r=S[e.replace(/-/g,`_`)];if(!r)return``;let i=v(r);return`<span class="ol-help-icon${t?` tooltip-below`:``}" data-tooltip="${i}" aria-label="帮助">${n(`info`)}</span>`}async function w(e){let t={};try{let e=await g(`/api/webui/config/openlist`);e.success&&e.config&&(t=e.config)}catch{}try{let e=await g(`/api/openlist/strm-engines`);e.success&&(d.availableEngines=e.engines||[])}catch{d.availableEngines=[]}d.strmEngines=[];try{t.strm_engines&&(d.strmEngines=JSON.parse(t.strm_engines))}catch{d.strmEngines=[]}d.strmEngines.length||(d.strmEngines=[{engine:``,monitored_paths:[]}]),d.configured=!!(t.webdav_host&&t.webdav_host.trim()),F();let r=[];try{t.refresh_paths&&(r=JSON.parse(t.refresh_paths))}catch{r=[]}d.refreshPaths=r.slice();function i(e,t,n,r,i=`text`,a=!1,o=!1){return p(e,t,n,{placeholder:r,type:i,persistLabel:a,readOnly:o,helpIcon:C(e.replace(/^ol-/,``))})}function a(e,t,n,r,i=!1){let a=n!=null&&String(n).trim()!==``,o=a||i?`floating-label is-shown is-floating${a?` is-filled`:``}`:`floating-label`,s=i?` data-persist-label="1"`:``,c=r.map(e=>{let t=e.value===n?` selected`:``;return`<option value="${v(e.value)}"${t}>${v(e.label)}</option>`}).join(``);return`
    ...
    <div class="field-grid">
      ...
      ${a(`ol-refresh-log-level`,`刷新日志级别`,t.refresh_log_level||`INFO`,[{value:`DEBUG`,label:`DEBUG — 调试`},{value:`INFO`,label:`INFO — 信息（推荐）`},{value:`WARNING`,label:`WARNING — 警告`}])}
    </div>
  ...
  </div>`;let b=d.strmEngines.filter(e=>e.engine),x=[];b.forEach(e=>{let t=d.availableEngines.find(t=>t.mount_path===e.engine);t&&t.local_path&&!x.includes(t.local_path)&&x.push(t.local_path)}),l+=`<div class="config-section"><h3>${n(`folder`)} A↔B 目录映射</h3>
    ...
    <div class="ab-mappings-display" id="ab-mappings-display">
      ${x.length?x.map((e,n)=>{let r=``;if(t.a_b_mappings)try{let n=JSON.parse(t.a_b_mappings).find(t=>t.a_root===e);n&&(r=n.b_root||``)}catch{}return`
