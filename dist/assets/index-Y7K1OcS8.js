const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./dashboard-Cj7GeWNC.js","./core-BOG7mpy_.js"])))=>i.map(i=>d[i]);
import{A as e,C as t,I as n,L as r,M as i,N as a,P as o,R as s,S as c,c as l,d as u,g as d,h as f,i as p,k as m,m as h,n as g,o as _,p as v,r as y,u as b,z as x}from"./core-BOG7mpy_.js";(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var S={webdav_host:`OpenList 的 WebDAV 服务地址。
格式：http://IP:端口/dav
...
修改并保存后，重启 WebUI / 主程序即按此路径与上面设置的级别写日志。`};function C(e,t=!1){let r=S[e.replace(/-/g,`_`)];if(!r)return``;let i=_(r);return`<span class="ol-help-icon${t?` tooltip-below`:``}" data-tooltip="${i}" aria-label="帮助">${n(`info`)}</span>`}async function w(e,t){let r={};try{let e=await h(`/api/webui/config/openlist`);e.success&&e.config&&(r=e.config)}catch{}try{let e=await h(`/api/openlist/strm-engines`);e.success&&(d.availableEngines=e.engines||[])}catch{d.availableEngines=[]}d.strmEngines=[];try{r.strm_engines&&(d.strmEngines=JSON.parse(r.strm_engines))}catch{d.strmEngines=[]}d.strmEngines.length||(d.strmEngines=[{engine:``,monitored_paths:[]}]),d.configured=!!(r.webdav_host&&r.webdav_host.trim()),F();let i=[];try{r.refresh_paths&&(i=JSON.parse(r.refresh_paths))}catch{i=[]}d.refreshPaths=i.slice();function a(e,t,n,r,i=`text`,a=!1,o=!1){return p(e,t,n,{placeholder:r,type:i,persistLabel:a,readOnly:o,helpIcon:C(e.replace(/^ol-/,``))})}function o(e,t,n,r,i=!1){let a=n!=null&&String(n).trim()!==``,o=a||i?`floating-label is-shown is-floating${a?` is-filled`:``}`:`floating-label`,s=i?` data-persist-label="1"`:``,c=r.map(e=>{let t=e.value===n?` selected`:``;return`<option value="${_(e.value)}"${t}>${_(e.label)}</option>`}).join(``);return`
      <div class="floating-field" data-field="${e}">
        <div class="field-control">
          <label class="${o}" data-role="label" for="${e}">${_(t)}${C(e.replace(/^ol-/,``))}</label>
          <select id="${e}" class="ol-select"${s}>${c}</select>
        </div>
      </div>`}function s(e,t,n){return`<div class="toggle-row">
      <span>${_(t)}${C(e.replace(/^ol-/,``))}</span>
    ...
    </div>`}function c(){let e=`<div class="strm-engine-wrap"><span class="monitored-paths-help">`+C(`monitored_paths`)+`</span>`;e+=`<table class="strm-engine-table"><thead><tr><th style="width:35%">STRM 引擎入口</th><th>监控目录</th><th style="width:60px">操作</th></tr></thead><tbody>`,d.strmEngines.forEach((t,n)=>{e+=l(n,t,d.strmEngines.length)}),e+=`</tbody></table>`;let t=d.configured?``:` disabled`;return e+=`<div class="table-actions">
      <button class="table-btn primary" id="add-engine-row"${t}>+ 添加行</button>
    </div></div>`,e}function l(e,t,n){let r=`<option value="">选择引擎...</option>`,i=new Set(d.strmEngines.map((t,n)=>n===e?``:t.engine).filter(Boolean));d.availableEngines.forEach(e=>{let n=t.engine===e.mount_path?` selected`:``,a=!n&&i.has(e.mount_path)?` disabled`:``;r+=`<option value="${_(e.mount_path)}"${n}${a}>${_(e.mount_path)}</option>`});let a=d.configured?``:` disabled`,o=``;return t.monitored_paths&&t.monitored_paths.length&&t.monitored_paths.forEach((t,n)=>{o+=`<span class="tag">${_(t)}<button class="tag-remove" data-row="${e}" data-pi="${n}" title="删除">×</button></span>`}),`<tr data-row-idx="${e}">
      <td>
    ...
    </tr>`}let u=`<div class="openlist-form">`;u+=`<div class="config-section"><h3>OpenList 连接</h3>
    ...
    <div class="field-grid">
      ${a(`ol-webdav-host`,`WebDAV 地址`,r.webdav_host||``,`http://127.0.0.1:5244/dav`)}
      ${a(`ol-webdav-user`,`用户名`,r.webdav_user||``,`admin`)}
      ${a(`ol-webdav-password`,`密码`,r.webdav_password||``,`输入密码`,`password`)}
      ${a(`ol-webdav-totp-secret`,`2FA 密钥`,r.webdav_totp_secret||``,`留空则不使用 2FA`,`password`)}
      ${a(`ol-b-root`,`B 区根目录`,r.b_root||t.b_root||``,`C:/path/to/b`)}
      ${a(`ol-c-root`,`C 区根目录`,r.c_root||t.c_root||``,`C:/path/to/c`)}
    </div>
  ...
  </div>`,u+=`<div class="config-section"><h3>${n(`area_a`)} STRM 引擎配置</h3>
    ...
    <div id="strm-engine-table-wrap">${c()}</div>
  </div>`,u+=`<div class="openlist-2col-grid">`;let f=(r.refresh_enabled||`true`).toLowerCase()===`true`,m=r.refresh_interval_minutes||`10`,g=r.refresh_depth||`5`;u+=`<div class="config-section"><h3>${n(`refresh`)} 刷新配置</h3>
    ${s(`ol-refresh-enabled`,`启用主动刷新`,f)}
    <div class="field-grid">
      ${a(`ol-refresh-interval`,`刷新间隔 (分钟)`,m,`10`,`number`)}
      ${a(`ol-refresh-depth`,`刷新深度`,g,`5`,`number`)}
    </div>
  ...
  </div>`;let v=(r.behavior_sync_on_startup||`false`).toLowerCase()===`true`,y=r.behavior_action||`MOVE`;u+=`<div class="config-section"><h3>${n(`settings`)} 行为配置</h3>
    <div class="field-grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:6px 12px">
      ${o(`ol-action`,`删除动作`,y,[{value:`MOVE`,label:`MOVE — 移动到回收站（推荐）`},{value:`DELETE`,label:`DELETE — 直接删除（危险）`}])}
      ${a(`ol-trash-dir`,`回收站目录名`,r.behavior_trash_dir_name||`trash`,`strm_回收站`)}
      ${a(`ol-ghost-protect`,`Ghost 保护时间 (秒)`,r.behavior_ghost_protect_seconds||`300`,`300`,`number`)}
      ${a(`ol-restore-delay`,`A→B 恢复延迟 (秒)`,r.behavior_a_to_b_restore_delay_seconds||`30`,`30`,`number`)}
      ${a(`ol-startup-wait`,`启动等待时间 (秒)`,r.behavior_sync_on_startup_wait||`0`,`0`,`number`)}
    </div>
    ${s(`ol-sync-startup`,`启动时全量同步`,v)}
  </div>`;let b=r.log_level||`INFO`;u+=`<div class="config-section"><h3>${n(`log`)} 日志配置</h3>
    <div class="field-grid">
      ${o(`ol-log-level`,`日志级别`,b,[{value:`DEBUG`,label:`DEBUG — 调试`},{value:`INFO`,label:`INFO — 信息（推荐）`},{value:`WARNING`,label:`WARNING — 警告`},{value:`ERROR`,label:`ERROR — 错误`}])}
      ${a(`ol-log-max-size`,`日志最大大小 (MB)`,r.log_max_size_mb||`2`,`2`,`number`)}
      ${a(`ol-log-backup-count`,`历史日志数量`,r.log_backup_count||`5`,`5`,`number`)}
      ${a(`ol-log-path`,`日志保存路径`,r.log_file||``,`logs/strm_bridge.log`)}
    </div>
  </div>`;let x=d.strmEngines.filter(e=>e.engine),S=[];x.forEach(e=>{let t=d.availableEngines.find(t=>t.mount_path===e.engine);t&&t.local_path&&!S.includes(t.local_path)&&S.push(t.local_path)}),u+=`<div class="config-section"><h3>${n(`folder`)} A 区文件夹 (只读)</h3>
    ...
    <div class="a-folders-display" id="a-folders-display">
      ${S.length?S.map(e=>`<span class="a-folder-chip">${_(e)}</span>`).join(``):`<span style="color:var(--text-muted)">暂无数据（请先配置 STRM 引擎）</span>`}
    </div>
  </div>`,u+=`</div>`,u+=`</div>`;let w=document.getElementById(`config-subpage`);if(w){let e=w.querySelector(`.config-back-btn`);e?e.insertAdjacentHTML(`afterend`,u):w.innerHTML=u;let t=w.querySelector(`.loading`);t&&t.remove()}D(t,r),E(),A()}function T(){let e=new Set;d.strmEngines.forEach(t=>{if(!t||!t.engine||!t.monitored_paths)return;let n=t.engine;t.monitored_paths.forEach(t=>{let r=String(t).replace(/\/$/,``).split(`/`).pop();if(r){let t=`${n.replace(/\/$/,``)}/${r}`;e.add(t)}})});let t=d.refreshPaths||[],n=new Set(t);for(let r of e)n.has(r)||t.push(r);d.refreshPaths=t,E()}function E(){let e=document.getElementById(`refresh-paths-tags`);if(!e)return;let t=d.refreshPaths||[];if(!t.length){e.innerHTML=`<span class="refresh-paths-empty">暂无刷新路径，可手动添加</span>`;return}e.innerHTML=t.map((e,t)=>`<span class="tag">${_(e)}<button class="tag-remove" data-path-idx="${t}" title="删除">×</button></span>`).join(``),e._delegatedListener||(e._delegatedListener=t=>{let n=t.target.closest(`.tag-remove`);if(!n||!e.contains(n))return;let r=parseInt(n.dataset.pathIdx);(d.refreshPaths&&d.refreshPaths[r])!==void 0&&(d.refreshPaths.splice(r,1),E())},e.addEventListener(`click`,e._delegatedListener))}function D(e,t){document.querySelectorAll(`.openlist-form .floating-field input, .openlist-form .floating-field select`).forEach(e=>{let t=e.closest(`.floating-field`),n=t&&t.querySelector(`.floating-label`);if(!n)return;let r=()=>{let t=String(e.value||``).trim()!==``,r=document.activeElement===e,i=e.dataset.persistLabel===`1`;n.classList.toggle(`is-shown`,t||r||i),n.classList.toggle(`is-floating`,t||r||i),n.classList.toggle(`is-filled`,(t||i)&&!r),e.classList.toggle(`has-value`,t)};e.addEventListener(`focus`,r),e.addEventListener(`blur`,r),e.addEventListener(`input`,r),r()}),[`ol-webdav-password`,`ol-webdav-totp-secret`].forEach(e=>{let t=document.getElementById(e);t&&(t.addEventListener(`focus`,function(){this.type=`text`}),t.addEventListener(`blur`,function(){this.type=`password`}))});let r=e=>{let t=document.getElementById(e);if(!t)return;let n=t.querySelectorAll(`button`);n.forEach(e=>{e.addEventListener(`click`,()=>{n.forEach(t=>t.classList.toggle(`active`,t===e))})})};r(`ol-refresh-enabled`),r(`ol-sync-startup`),document.getElementById(`ol-test-connection`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`ol-test-connection`);e.disabled=!0,e.innerHTML=`测试中...`;try{let e=await h(`/api/openlist/test-connection`,{method:`POST`,body:{host:document.getElementById(`ol-webdav-host`)?.value||``,user:document.getElementById(`ol-webdav-user`)?.value||``,password:document.getElementById(`ol-webdav-password`)?.value||``,totp_secret:document.getElementById(`ol-webdav-totp-secret`)?.value||``}});if(e.success){y(`连接成功！`,`success`),d.apiStatus=`online`;try{let e=await h(`/api/openlist/strm-engines`);e.success&&(d.availableEngines=e.engines||[],j())}catch{}}else{d.apiStatus={wrong_password:`auth_failed_password`,wrong_2fa:`auth_failed_2fa`,account_not_found:`auth_failed`,network_error:`offline`,exception:`offline`}[e.error_type||`unknown`]||`auth_failed`,y(`连接失败: `+(e.error||`未知错误`),`error`),document.querySelectorAll(`.engine-select`).forEach(e=>e.disabled=!0);let t=document.getElementById(`add-engine-row`);t&&(t.disabled=!0)}}catch(e){y(`测试失败: `+e.message,`error`)}finally{e.disabled=!1,e.innerHTML=`测试连接`}}),document.getElementById(`ol-restart-webui`)?.addEventListener(`click`,async()=>{g(`重启 Bridge 主程序`,`确定要重启 Bridge 主程序吗？

重启将重新加载 STRM 存储映射，期间 WebUI 将短暂不可用（约 3-5 秒）。`,async()=>{let e=document.getElementById(`ol-restart-webui`);e.disabled=!0,e.innerHTML=`重启中...`,y(`正在重启主程序，请稍候...`,`info`);try{let t=await h(`/api/restart-webui`,{method:`POST`});t.success?(y(`主程序正在重启，请稍候刷新页面...`,`success`),setTimeout(()=>{window.location.reload()},4e3)):(y(`重启失败: `+(t.message||`未知错误`),`error`),e.disabled=!1,e.innerHTML=`${n(`refresh`)} 重启Bridge主程序`)}catch(t){y(`重启请求失败: `+t.message,`error`),e.disabled=!1,e.innerHTML=`${n(`refresh`)} 重启Bridge主程序`}})});let i=document.getElementById(`strm-engine-table-wrap`);i&&!i._delegatedTagRemoveListener&&(i._delegatedTagRemoveListener=e=>{let t=e.target.closest(`.tag-remove`);if(!t||!i.contains(t))return;let n=parseInt(t.dataset.row),r=parseInt(t.dataset.pi);d.strmEngines[n]&&d.strmEngines[n].monitored_paths&&(d.strmEngines[n].monitored_paths.splice(r,1),k(n),T())},i.addEventListener(`click`,i._delegatedTagRemoveListener)),document.querySelectorAll(`[data-delete-row]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.deleteRow);d.strmEngines.length<1||(d.strmEngines.splice(t,1),j(),T())})}),document.getElementById(`add-engine-row`)?.addEventListener(`click`,()=>{d.strmEngines.push({engine:``,monitored_paths:[]}),j(),T()}),document.getElementById(`add-refresh-path-btn`)?.addEventListener(`click`,()=>{let e=document.getElementById(`refresh-path-input`);if(!e)return;let t=e.value.trim();if(t){if(d.refreshPaths||=[],d.refreshPaths.includes(t)){y(`该路径已存在`,`info`);return}d.refreshPaths.push(t),e.value=``,E()}}),document.getElementById(`refresh-path-input`)?.addEventListener(`keydown`,e=>{e.key===`Enter`&&document.getElementById(`add-refresh-path-btn`)?.click()}),document.getElementById(`ol-save`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`ol-save`);e.disabled=!0,e.innerHTML=`保存中...`;try{let e=await h(`/api/webui/config/openlist`,{method:`POST`,body:{webdav_host:document.getElementById(`ol-webdav-host`)?.value||``,webdav_user:document.getElementById(`ol-webdav-user`)?.value||``,webdav_password:document.getElementById(`ol-webdav-password`)?.value||``,webdav_totp_secret:document.getElementById(`ol-webdav-totp-secret`)?.value||``,b_root:document.getElementById(`ol-b-root`)?.value||``,c_root:document.getElementById(`ol-c-root`)?.value||``,refresh_paths:JSON.stringify(d.refreshPaths||[]),strm_engines:JSON.stringify(d.strmEngines),refresh_enabled:document.querySelector(`#ol-refresh-enabled button[data-value="on"].active`)?`true`:`false`,refresh_interval_minutes:document.getElementById(`ol-refresh-interval`)?.value||`10`,refresh_depth:document.getElementById(`ol-refresh-depth`)?.value||`5`,behavior_action:document.getElementById(`ol-action`)?.value||`MOVE`,behavior_trash_dir_name:document.getElementById(`ol-trash-dir`)?.value||`trash`,behavior_ghost_protect_seconds:document.getElementById(`ol-ghost-protect`)?.value||`300`,behavior_a_to_b_restore_delay_seconds:document.getElementById(`ol-restore-delay`)?.value||`30`,behavior_sync_on_startup:document.querySelector(`#ol-sync-startup button[data-value="on"].active`)?`true`:`false`,behavior_sync_on_startup_wait:document.getElementById(`ol-startup-wait`)?.value||`0`,log_level:document.getElementById(`ol-log-level`)?.value||`INFO`,log_max_size_mb:document.getElementById(`ol-log-max-size`)?.value||`2`,log_backup_count:document.getElementById(`ol-log-backup-count`)?.value||`5`,log_file:document.getElementById(`ol-log-path`)?.value||``}});e.success?(y(`OpenList 配置已保存并热更新`,`success`),d.configured=!!(document.getElementById(`ol-webdav-host`)?.value||``).trim(),F(),await M(),O()):y(`保存失败: `+(e.error||`未知错误`),`error`)}catch(e){y(`保存失败: `+e.message,`error`)}finally{e.disabled=!1,e.innerHTML=`保存`}})}function O(){let e=document.getElementById(`a-folders-display`);if(!e)return;let t=d.strmEngines.filter(e=>e.engine),n=[];t.forEach(e=>{let t=d.availableEngines.find(t=>t.mount_path===e.engine);t&&t.local_path&&!n.includes(t.local_path)&&n.push(t.local_path)}),e.innerHTML=n.length?n.map(e=>`<span class="a-folder-chip">${_(e)}</span>`).join(``):`<span style="color:var(--text-muted)">暂无数据（请先配置 STRM 引擎）</span>`}function k(e){let t=document.getElementById(`tag-container-${e}`);if(!t)return;let n=d.strmEngines[e];if(!n||!n.monitored_paths||!n.monitored_paths.length){t.innerHTML=`<span style="color:var(--text-muted);font-size:calc(var(--font-base) - 1px)">选择引擎后自动填充</span>`;return}t.innerHTML=n.monitored_paths.map((t,n)=>`<span class="tag">${_(t)}<button class="tag-remove" data-row="${e}" data-pi="${n}" title="删除">×</button></span>`).join(``)}function A(){document.querySelectorAll(`.engine-select`).forEach(e=>{e.dataset.bound||(e.dataset.bound=`1`,e.addEventListener(`change`,async()=>{let t=parseInt(e.dataset.row),n=e.value;if(!n)return;let r=d.availableEngines.find(e=>e.mount_path===n);if(r)d.strmEngines[t]={engine:n,monitored_paths:r.paths||[]},k(t),T();else try{let e=await h(`/api/openlist/monitored-paths?engine=${encodeURIComponent(n)}`);e.success&&(d.strmEngines[t]={engine:n,monitored_paths:e.paths||[]},k(t),T())}catch{}}))})}function j(){let e=document.getElementById(`strm-engine-table-wrap`);if(!e)return;let t=`<table class="strm-engine-table"><thead><tr><th style="width:35%">STRM 引擎入口</th><th>监控目录</th><th style="width:60px">操作</th></tr></thead><tbody>`;d.strmEngines.forEach((e,n)=>{let r=`<option value="">选择引擎...</option>`,i=new Set(d.strmEngines.map((e,t)=>t===n?``:e.engine).filter(Boolean));d.availableEngines.forEach(t=>{let n=e.engine===t.mount_path?` selected`:``,a=!n&&i.has(t.mount_path)?` disabled`:``;r+=`<option value="${_(t.mount_path)}"${n}${a}>${_(t.mount_path)}</option>`});let a=d.configured?``:` disabled`,o=``;e.monitored_paths&&e.monitored_paths.length&&e.monitored_paths.forEach((e,t)=>{o+=`<span class="tag">${_(e)}<button class="tag-remove" data-row="${n}" data-pi="${t}" title="删除">×</button></span>`}),t+=`<tr data-row-idx="${n}">
