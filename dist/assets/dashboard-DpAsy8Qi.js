...
${d.watchers_healthy===!1?`<div class="dashboard-warning-banner" style="margin:12px 0;padding:10px 14px;background:color-mix(in srgb,var(--error) 12%,transparent);border:1px solid color-mix(in srgb,var(--error) 40%,transparent);border-radius:var(--radius-control);color:var(--error);font-size:13px;display:flex;align-items:center;gap:8px">${t(`warn`)} watchdog 监视器降级：部分区域事件可能未同步，请检查 WebUI 日志</div>`:``}

<div class="stat-grid">
  <div class="stat-card"><div class="label">${t(`movie`)} A 区 STRM</div><div class="value">${d.a_count}</div></div>
  <div class="stat-card"><div class="label">${t(`tv`)} B 区 STRM</div><div class="value">${d.b_count}</div></div>
  <div class="stat-card"><div class="label">${t(`area_c`)} C 区幽灵</div><div class="value">${d.c_count}</div></div>
<div class="stat-card"><div class="label">B - valid</div><div class="value stat-value-primary">${d.b_valid}</div></div>
  ...
  <div class="stat-card"><div class="label">${t(`tmdb`)} TMDB</div><div class="value stat-value-large">${d.tmdb_configured?`已配置`:`未配置`}</div></div>
  <div class="stat-card"><div class="label">WebUI 运行时间</div><div class="value stat-value-large" id="uptime-val">-</div></div>
</div>

<!-- 索引元数据（Task 2） -->
<div class="stat-grid" style="margin-top:16px">
  <div class="stat-card"><div class="label">${t(`sync`)} 索引代次</div><div class="value stat-value-primary" id="index-generation">#${d.index_metadata?.index_generation||0}</div></div>
  <div class="stat-card"><div class="label">${t(`speed`)} 最近索引</div><div class="value" title="${p(d.index_metadata?.last_full_index_at)}">${d.index_metadata?.last_full_index_at?r(d.index_metadata.last_full_index_at):`暂无记录`}</div></div>
  <div class="stat-card"><div class="label">${t(`swap_horiz`)} 映射版本</div><div class="value" title="${n(d.index_metadata?.mapping_version||``)}">${d.index_metadata?.mapping_version?d.index_metadata.mapping_version.substring(0,8)+`...`:`-`}</div></div>
  <div class="stat-card"><div class="label">映射版本生成</div><div class="value" title="${p(d.index_metadata?.mapping_version_generated_at)}">${d.index_metadata?.mapping_version_generated_at?r(d.index_metadata.mapping_version_generated_at):`暂无记录`}</div></div>
</div>
    ...
    ${d.mappings.map(e=>`
      <div style="background:var(--bg-surface-variant);padding:12px;border-radius:8px;border:1px solid var(--border-subtle)">
        ...
        <div style="font-size:12px;color:var(--text-secondary);margin-bottom:4px">
          <div>A: ${n(m(e.a_root))}</div>
          <div>B: ${n(m(e.b_root))}</div>
        </div>
        <div style="font-size:11px;color:var(--text-muted)">
          索引时间: <span title="${p(e.index_generation_at)}">${e.index_generation_at?r(e.index_generation_at):`未索引`}</span>
        </div>
      </div>
    `).join(``)}
  </div>
...
`:``}
    ...
    </div>`;function p(e){if(!e||e===0)return`暂无记录`;try{let t=new Date(e*1e3),n=e=>String(e).padStart(2,`0`);return`${t.getFullYear()}-${n(t.getMonth()+1)}-${n(t.getDate())} ${n(t.getHours())}:${n(t.getMinutes())}:${n(t.getSeconds())}`}catch{return`暂无记录`}}function m(e){if(!e)return`/`;let t=e.split(`/`).filter(Boolean);return t.length<=2?`/`+t.join(`/`):`/`+t.slice(0,2).join(`/`).replace(/\/$/,``)+`/...`}document.getElementById(`main-start-btn`)?.addEventListener(`click`,S),document.getElementById(`main-stop-btn`)?.addEventListener(`click`,C);let h=document.getElementById(`onboarding-quick-btn`);h&&h.addEventListener(`click`,async()=>{try{await s(`/api/webui/config/ui`,{method:`POST`,body:JSON.stringify({onboarding_completed:`0`})})}catch(e){console.error(`Failed to reset onboarding:`,e)}await v()});let g=document.getElementById(`btn-run-full-audit`),_=document.getElementById(`audit-status-text`);g&&g.addEventListener(`click`,async()=>{let e=o();if(confirm(`确定要执行全量审计吗？

这是一个重操作，耗时取决于 A 区库大小，会扫描全部 A 区根目录（含机械硬盘）。不会删除任何文件。`)){g.disabled=!0,g.innerHTML=`审计中...`,_&&(_.textContent=`正在启动审计...`);try{if((await s(`/api/index/audit`,{method:`POST`})).status===`already_running`){_&&(_.textContent=`审计已在进行中`),g.disabled=!1,g.innerHTML=`${t(`refresh`)} 立即全量审计`;return}for(let n=0;n<300;n++){if(await new Promise(e=>setTimeout(e,2e3)),e())return;try{let e=await s(`/api/index/audit/status`);if(e.result&&e.result.status===`already_running`){_&&(_.textContent=`审计被其他任务占用（已在进行中）`),g.disabled=!1,g.innerHTML=`${t(`refresh`)} 立即全量审计`;return}if(!e.running&&e.result){e.result.status===`completed`?_&&(_.textContent=`审计完成，索引代次 #`+(e.result.index_generation||0)):e.result.error?_&&(_.textContent=`审计失败: `+e.result.error):_&&(_.textContent=`审计未完成`),g.disabled=!1,g.innerHTML=`${t(`refresh`)} 立即全量审计`;try{let e=await s(`/api/dashboard`);if(e&&e.index_metadata){let t=document.getElementById(`index-generation`);t&&(t.textContent=`#`+(e.index_metadata.index_generation||0))}}catch{}return}_&&(_.textContent=`审计进行中... (`+n*2+`s)`)}catch{}}_&&(_.textContent=`审计超时，请稍后重试`),g.disabled=!1,g.innerHTML=`${t(`refresh`)} 立即全量审计`}catch(e){_&&(_.textContent=`审计请求失败: `+e.message),g.disabled=!1,g.innerHTML=`${t(`refresh`)} 立即全量审计`}}}),v(),x(),e(),a&&clearInterval(a),f(setInterval(x,l.MAIN_STATUS_POLL_INTERVAL))}export{w as renderDashboard,x as updateMainStatus,c as updateUptime};