const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./dashboard-A0Wxagou.js","./core-iN5uWBSS.js"])))=>i.map(i=>d[i]);
import{F as e,I as t,L as n,M as r,N as i,O as a,R as o,S as s,c,d as l,f as u,h as d,i as f,j as p,k as m,m as h,n as g,o as _,p as v,r as y,u as b,x}from"./core-iN5uWBSS.js";(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var S={webdav_host:`OpenList 的 WebDAV 服务地址。
格式：http://IP:端口/dav
...
留空使用默认值。
修改并保存后，重启 WebUI / 主程序即按此路径与上面设置的级别写日志。`};function C(t,n=!1){let r=S[t.replace(/-/g,`_`)];if(!r)return``;let i=_(r);return`<span class="ol-help-icon${n?` tooltip-below`:``}" data-tooltip="${i}" aria-label="帮助">${e(`info`)}</span>`}async function w(t){let n={};try{let e=await v(`/api/webui/config/openlist`);e.success&&e.config&&(n=e.config)}catch{}try{let e=await v(`/api/openlist/strm-engines`);e.success&&(d.availableEngines=e.engines||[])}catch{d.availableEngines=[]}d.strmEngines=[];try{n.strm_engines&&(d.strmEngines=JSON.parse(n.strm_engines))}catch{d.strmEngines=[]}d.strmEngines.length||(d.strmEngines=[{engine:``,monitored_paths:[]}]),d.configured=!!(n.webdav_host&&n.webdav_host.trim()),F();let r=[];try{n.refresh_paths&&(r=JSON.parse(n.refresh_paths))}catch{r=[]}d.refreshPaths=r.slice();function i(e,t,n,r,i=`text`,a=!1,o=!1){return f(e,t,n,{placeholder:r,type:i,persistLabel:a,readOnly:o,helpIcon:C(e.replace(/^ol-/,``))})}function a(e,t,n,r,i=!1){let a=n!=null&&String(n).trim()!==``,o=a||i?`floating-label is-shown is-floating${a?` is-filled`:``}`:`floating-label`,s=i?` data-persist-label="1"`:``,c=r.map(e=>{let t=e.value===n?` selected`:``;return`<option value="${_(e.value)}"${t}>${_(e.label)}</option>`}).join(``);return`
    ...
    <div class="field-grid">
      ...
      ${i(`ol-b-root`,`B 区根目录`,n.b_root||t.b_root||``,`请填写 B 区根目录的绝对路径`)}
      ${i(`ol-c-root`,`C 区根目录`,n.c_root||t.c_root||``,`请填写 C 区根目录的绝对路径`)}
    </div>
    <div class="openlist-top-actions">
      ...
      <button class="toolbar-btn secondary" id="ol-restart-webui">${e(`refresh`)} 重启Bridge主程序</button>
      <span class="test-connection-hint">openlist api 测试通过，再保存, 必须使用管理员账户登录</span>
    </div>
  </div>`,l+=`<div class="config-section"><h3>${e(`area_a`)} STRM 引擎配置</h3>
  ...
  </div>`,l+=`<div class="openlist-2col-grid">`;let u=(n.refresh_enabled||`true`).toLowerCase()===`true`,p=n.refresh_interval_minutes||`10`,m=n.refresh_depth||`5`;l+=`<div class="config-section"><h3>${e(`refresh`)} 刷新配置</h3>
    ${o(`ol-refresh-enabled`,`启用主动刷新`,u)}
    <div class="field-grid">
      ${i(`ol-refresh-interval`,`刷新间隔 (分钟)`,p,`10`,`number`)}
  ...
  </div>`;let h=(n.behavior_sync_on_startup||`false`).toLowerCase()===`true`,g=n.behavior_action||`MOVE`;l+=`<div class="config-section"><h3>${e(`settings`)} 行为配置</h3>
    <div class="field-grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:6px 12px">
      ${a(`ol-action`,`删除动作`,g,[{value:`MOVE`,label:`MOVE — 移动到回收站（推荐）`},{value:`DELETE`,label:`DELETE — 直接删除（危险）`}])}
  ...
  </div>`;let y=n.log_level||`INFO`;l+=`<div class="config-section"><h3>${e(`log`)} 日志配置</h3>
    <div class="field-grid">
      ...
      ${i(`ol-log-path`,`日志保存路径`,n.log_file||t.log_file||``,`strm_bridge.log`)}
    </div>
  </div>`;let b=d.strmEngines.filter(e=>e.engine),x=[];b.forEach(e=>{let t=d.availableEngines.find(t=>t.mount_path===e.engine);t&&t.local_path&&!x.includes(t.local_path)&&x.push(t.local_path)}),l+=`<div class="config-section"><h3>${e(`folder`)} A 区文件夹 (只读)</h3>
