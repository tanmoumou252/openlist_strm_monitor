...
如果 OpenList 开启了两步验证，需要填写此字段。`,b_root:`媒体库实际扫描的目录，程序会把 A 区 STRM 同步到这里。`,c_root:`用于幽灵迁移、异常隔离等场景。`,monitored_paths:`该引擎下需要监控的目录（仅用于引擎配置与 A 区派生）。选择引擎入口后会自动填充，可删除不需要的目录。
注意：这里与下方"刷新配置"中的主动刷新路径是独立维护的，互不影响。`,refresh_paths:`主动刷新路径列表（JSON 数组），独立维护。
程序会定期调用 API 刷新这些路径，让 OpenList 重新扫描目录。
注意：此列表不会从上方引擎监控目录自动推导，删除这里某条路径也不会影响引擎配置。`,strm_engines:`STRM 引擎配置。
