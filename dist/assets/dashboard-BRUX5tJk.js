...
<div class="stat-grid" style="margin-top:16px">
  <div class="stat-card"><div class="label">${t(`sync`)} 索引代次</div><div class="value stat-value-primary" id="index-generation">#${u.index_metadata?.index_generation||0}</div></div>
  <div class="stat-card"><div class="label">${t(`speed`)} 最近索引</div><div class="value" title="${d(u.index_metadata?.last_full_index_at)}">${u.index_metadata?.last_full_index_at?r(u.index_metadata.last_full_index_at):`暂无记录`}</div></div>
  <div class="stat-card"><div class="label">${t(`swap_horiz`)} 映射版本</div><div class="value" title="${n(u.index_metadata?.mapping_version||``)}">${u.index_metadata?.mapping_version?u.index_metadata.mapping_version.substring(0,8)+`...`:`-`}</div></div>
...
这是一个重操作，耗时取决于 A 区库大小，会扫描全部 A 区根目录（含机械硬盘）。不会删除任何文件。`)){h.disabled=!0,h.innerHTML=`审计中...`,g&&(g.textContent=`正在启动审计...`);try{if((await s(`/api/index/audit`,{method:`POST`})).status===`already_running`){g&&(g.textContent=`审计已在进行中`),h.disabled=!1,h.innerHTML=`${t(`refresh`)} 立即全量审计`;return}for(let e=0;e<300;e++){if(await new Promise(e=>setTimeout(e,2e3)),o())return;try{let n=await s(`/api/index/audit/status`);if(n.result&&n.result.status===`already_running`){g&&(g.textContent=`审计被其他任务占用（已在进行中）`),h.disabled=!1,h.innerHTML=`${t(`refresh`)} 立即全量审计`;return}if(!n.running&&n.result){n.result.status===`completed`?g&&(g.textContent=`审计完成，索引代次 #`+(n.result.index_generation||0)):n.result.error?g&&(g.textContent=`审计失败: `+n.result.error):g&&(g.textContent=`审计未完成`),h.disabled=!1,h.innerHTML=`${t(`refresh`)} 立即全量审计`;try{let e=await s(`/api/dashboard`);if(e&&e.index_metadata){let t=document.getElementById(`index-generation`);t&&(t.textContent=`#`+(e.index_metadata.index_generation||0))}}catch{}return}g&&(g.textContent=`审计进行中... (`+e*2+`s)`)}catch{}}g&&(g.textContent=`审计超时，请稍后重试`),h.disabled=!1,h.innerHTML=`${t(`refresh`)} 立即全量审计`}catch(e){g&&(g.textContent=`审计请求失败: `+e.message),h.disabled=!1,h.innerHTML=`${t(`refresh`)} 立即全量审计`}}}),v(),x(),e(),a&&clearInterval(a),f(setInterval(x,l.MAIN_STATUS_POLL_INTERVAL))}export{w as renderDashboard,x as updateMainStatus,c as updateUptime};