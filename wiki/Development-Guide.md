...
- **新手引导端到端**：分两层覆盖——①**##26 七步全链路**：`test_e2e_full_flow.py` 的 `test_complete_seven_step_onboarding`（正向 HTTP 全链路）+ `TestSevenStepFailureReasons`（每步失败原因/成功条件）+ 接线回归用例（`test_tmdb_config_reinitializes_client` / `test_mapping_id_autogenerated_on_config_save` / `test_openlist_config_triggers_storage_reload`）；②**引导单步跟踪与预检**：`test_onboarding_e2e.py`（`view_ab` / `tmdb_refresh` / `tmdb_match` 步骤完成、`config/validate` 预检、完整旅程与复位、配置联动）。七步全链路不在 `test_onboarding_e2e.py` 中。
...
默认 `0.0.0.0:8579`(实际端口由 `config.toml` 的 `[webui].port` 决定)使 WebUI 在局域网可访问。仅本地访问时改为 `127.0.0.1`。